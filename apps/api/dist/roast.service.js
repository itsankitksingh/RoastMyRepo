...
let RoastService = class RoastService {
    groqClient = null;
    async roast({ username, accessToken }) {
        if (!username) {
            throw new common_1.HttpException({ error: 'Username required' }, common_1.HttpStatus.BAD_REQUEST);
        }
        ...
        if (!userRes.ok) {
            if (userRes.status === 404) {
                throw new common_1.HttpException({ error: 'GitHub user not found. Double-check the username.' }, common_1.HttpStatus.NOT_FOUND);
            }
            if (userRes.status === 403 && userRes.headers.get('x-ratelimit-remaining') === '0') {
                throw new common_1.HttpException({
                    error: 'GitHub API rate limit hit. Try again in ~60 minutes, or configure GITHUB_TOKEN.',
                }, common_1.HttpStatus.TOO_MANY_REQUESTS);
            }
            throw new common_1.BadGatewayException({ error: `GitHub API error (${userRes.status}).` });
        }
        ...
        const suspiciousRepos = ownRepos
            .filter((repo) => suspiciousPatterns.some((pattern) => pattern.test(repo.name) || pattern.test(repo.description || '')))
            .map((repo) => repo.name);
        ...
        const uniqueRepos = Array.from(new Map([...topByStars, ...recentPushed, ...deployedProjects].map((repo) => [repo.name, repo])).values()).slice(0, 7);
        const repoDetails = await Promise.all(uniqueRepos.map(async (repo) => {
        ...
        const allRepoSummary = ownRepos.slice(0, 25).map((repo) => `${repo.name} (${repo.stargazers_count}⭐, ${repo.language || '?'}, ${Math.floor((Date.now() - new Date(repo.pushed_at).getTime()) / (1000 * 60 * 60 * 24))}d ago)`);
        let baseScore = 5.0;
        ...
        if (forkRatio > 0.6)
            baseScore += 0.5;
        if (ownRepos.length > 5 && languageDiversityCount <= 1)
            baseScore += 0.5;
        ...
        if (pushEventCount > 10)
            baseScore -= 0.3;
        if (pushEventCount > 25)
            baseScore -= 0.4;
        if (pushEventCount > 50)
            baseScore -= 0.5;
        const highSignalLangs = ['Rust', 'Go', 'Haskell', 'Erlang', 'C', 'C++', 'Zig', 'CUDA', 'Assembly', 'Solidity'];
        const hasHighSignal = topLanguages.some((lang) => highSignalLangs.some((highSignal) => lang.startsWith(highSignal)));
        if (hasHighSignal)
            baseScore -= 0.5;
        if (languageDiversityCount >= 3)
            baseScore -= 0.2;
        if (languageDiversityCount >= 5)
            baseScore -= 0.3;
        if (user.blog && user.blog.length > 5)
            baseScore -= 0.2;
        ...
        const techBase = Math.max(1.5, Math.min(9.5, Math.round((5.0 -
            starBonus * 0.7 +
            tutorialRatio * 1.5 +
            zeroStarRatio * 0.8 -
            (hasHighSignal ? 1.0 : 0) -
            ...
            notableBoost) *
            10) / 10));
        const aiBase = Math.max(1.5, Math.min(9.5, Math.round((5.5 +
            staleRatio * 1.5 +
            ...
            notableBoost * 0.5) *
            10) / 10));
        const moatBase = Math.max(1.5, Math.min(9.5, Math.round((5.0 +
            tutorialRatio * 2.0 -
            starBonus * 0.5 -
            ...
            notableBoost * 1.2) *
            10) / 10));
        const mktBase = Math.max(1.5, Math.min(9.5, Math.round((5.0 -
            (user.followers > 100 ? 0.3 : 0) -
...
Return only valid JSON:
{
  ...
  "subDescriptions": {
    ...
    async getReadme(fullName, headers) {
        try {
    ...
    async getRecentCommits(fullName, headers) {
        try {
    ...
    async getLanguages(fullName, headers) {
        try {
...
exports.RoastService = RoastService = __decorate([
